...
(defproject metabase-core "1.0.0-SNAPSHOT"
  ...
  {"bikeshed"                          ["bikeshed" "--max-line-length" "205"]
   ...
   "install-with-tests"                ["with-profile" "install-with-tests" "install"]
  ...
  {:test-dependencies
   ...
   [:test-dependencies
    {:auto-clean   true
     ...
     ;; we don't actually need to compile the tests themselves, so swap out the `expect` macro with a no-op macro that
     ;; ignores its args and resolves to `nil`
     :injections [(do
                    (require 'expectations)
                    (intern 'expectations 'expect (constantly nil))
                    (alter-meta! #'expectations/expect assoc :macro true)
                    (intern 'expectations 'doexpect (constantly nil))
                    (alter-meta! #'expectations/doexpect assoc :macro true))]}]
   ...
   :expectations
   {:injections     [(require 'metabase.test-setup                    ; for test setup stuff
