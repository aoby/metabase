...
(defn ^:private get-path-in-filesystem ^Path [^FileSystem filesystem, ^String path-component & more-components]
  (.getPath filesystem path-component (varargs String more-components)))

(defn get-path ^Path [& path-components]
...
(defn- copy! [^Path source, ^Path dest]
  (du/profile (str "EXTRACT " source " -> " dest) ; NOCOMMIT
